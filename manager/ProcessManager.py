...
## Copyright (c) 2017, The Sumokoin Project (www.sumokoin.org)
'''
Process managers for zephyrd, zephyr-wallet-cli and zephyr-wallet-rpc
'''
...
class ZephyrdManager(ProcessManager):
    def __init__(self, resources_path, log_level=0, block_sync_size=10):
        proc_args = u'%s/bin/zephyrd --log-level %d --block-sync-size %d' % (resources_path, log_level, block_sync_size)
        ProcessManager.__init__(self, proc_args, "zephyrd")
        self.synced = Event()
        self.stopped = Event()
...
class WalletCliManager(ProcessManager):
    ...
    def __init__(self, resources_path, wallet_file_path, wallet_log_path, restore_wallet=False):
        if not restore_wallet:
            wallet_args = u'%s/bin/zephyr-wallet-cli --generate-new-wallet=%s --log-file=%s' \
                                                % (resources_path, wallet_file_path, wallet_log_path)
        else:
            wallet_args = u'%s/bin/zephyr-wallet-cli --log-file=%s --daemon-port 44044 --restore-deterministic-wallet' \
                                                % (resources_path, wallet_log_path)
        ProcessManager.__init__(self, wallet_args, "zephyr-wallet-cli")
        self.ready = Event()
        self.last_error = ""
