...
from manager.ProcessManager import ZephyrdManager, WalletRPCManager
...
class BaseWebUI(QMainWindow):
    def __init__(self, html, app, hub, window_size, debug=False):
        ...
        self.icon = self._getQIcon('zephyr_icon_64.png')
...
class MainWebUI(BaseWebUI):
    def __init__(self, app, hub, debug):
        ...
        if sys.platform == 'darwin':
            tray_icon = 'zephyr_16x16_mac.png'
        elif sys.platform == "win32":
            tray_icon = 'zephyr_16x16.png'
        else:
            tray_icon = 'zephyr_32x32_ubuntu.png'
        ...
        self.zephyrd_daemon_manager = None
...
#         self.view.load(qt_core.QUrl(self.url))
    ...
    def start_deamon(self):
        #start zephyrd daemon
        self.zephyrd_daemon_manager = ZephyrdManager(self.app.property("ResPath"), 
                                            self.app_settings.settings['daemon']['log_level'], 
                                            self.app_settings.settings['daemon']['block_sync_size'])
        
        self.zephyrd_daemon_manager.start()
    ...
    def run_wallet_rpc(self, wallet_password, log_level=0):
        ...
        while True:
            ...
            zephyrd_info = self.daemon_rpc_request.get_info()
            if zephyrd_info['status'] == "OK":
    ...
    def _update_daemon_status(self):
        ...
        if zephyrd_info['status'] == "OK":
            status = "Connected"
            self.current_height = int(zephyrd_info['height'])
            target_height = int(zephyrd_info['target_height'])
        ...
        sync_status = "Disconnected" if zephyrd_info['status'] != "OK" else "Synchronizing..."
        if zephyrd_info['status'] == "OK" and self.current_height == self.target_height:
...
#             print(json.dumps(wallet_info, indent=4))
    ...
    def _handleAboutToQuit(self):
        ...
        if self.wallet_rpc_manager is not None:
            self.wallet_rpc_manager.stop()
        if self.zephyrd_daemon_manager is not None:
            self.zephyrd_daemon_manager.stop()
